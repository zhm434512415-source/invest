        ...
        <div className="p-7">
          ...
          <div className="space-y-5">
            ...
            <div className="grid grid-cols-2 gap-5">
              ...
              <label className="flex flex-col items-center justify-center bg-white dark:bg-[#2c2c2e] p-6 rounded-3xl shadow-sm cursor-pointer active:scale-95 transition-all"><Upload className="text-green-500 mb-3" size={28} /><span className="text-sm font-black text-main">导入恢复</span><input type="file" className="hidden" accept=".json" onChange={(e) => { const file = e.target.files?.[0]; if (!file) return; const reader = new FileReader(); reader.onload = (ev) => { try { const parsed = JSON.parse(ev.target?.result as string); const importedData = { goldCategories: parsed.goldCategories || [], fixedDeposits: parsed.fixedDeposits || [], referenceGoldPrice: parsed.referenceGoldPrice ?? 550, referenceUsdRate: parsed.referenceUsdRate ?? 7.0, demandDepositAmount: parsed.demandDepositAmount ?? 0, assetSnapshots: parsed.assetSnapshots || [] }; updateData(importedData); if (!importedData.goldCategories.find(c => c.name === PHYSICAL_GOLD_NAME)) { updateData(d => ({ ...d, goldCategories: [{ id: 'physical-gold-default', name: PHYSICAL_GOLD_NAME, records: [], isExpanded: true }, ...d.goldCategories] })); } setFdSort(parsed.fixedDepositSort ?? { field: 'startDate', order: 'desc' }); setTimeDimension(parsed.timeDimension ?? 'month'); setSavedTimeDimension(parsed.timeDimension ?? 'month'); setDarkMode(parsed.darkMode ?? true); setShowSettings(false); } catch (err) { alert("文件格式错误"); } }; reader.readAsText(file); }}/></label>
            </div>
          </div>
        </div>
      </div>
